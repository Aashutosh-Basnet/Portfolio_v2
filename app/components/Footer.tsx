...
const Footer: React.FC = () => {
  ...
  return (
    <footer className=" bg-gray-100 text-slate-700">
          ...
          <div className="flex items-center space-x-4">
            {socialLinks.map((link) => (
              <a
                ...
                className="p-2 bg-slate-200 rounded-full text-slate-700 hover:bg-slate-300 hover:text-primary-600 transition-all duration-300 transform hover:scale-110"
              >
                <link.icon size={20} />
              </a>
            ))}
            ...
            <nav className="flex space-x-6">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="text-sm hover:text-primary-600 hover:underline transition-colors duration-300"
                >
                  {link.name}
                </a>
              ))}
